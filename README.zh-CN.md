...
### ▶ [**立即体验**](https://yunyueli.github.io/telos/app/) —— 免安装，自带一个免费 key
...
上面的在线版直接在浏览器里跑完整闭环——输入任意目标，看 Telos 倒推出一张完整、分阶段的知识地图。无需安装——首次粘一个免费 LLM key（随请求发往端点、不落盘，并随账号同步），即可开始。
...
| **要 key 吗** | 要——免费，粘一次 | 要（免费 DeepSeek） | 要（你的用户各自带） |
...
- **在线版（BYOK）：** 在「设置 · 接入状态」粘你自己的免费 LLM key——随每次请求发往倒推 Worker（端点不落盘）、存在本机、并随账号同步，跨设备带着走。
