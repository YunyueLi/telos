// Telos prompt 库（Worker 端）—— 倒推 / 微课 / 诊断的全部提示词。
//
// 【公开 baseline + 私有增强】本文件是「够用、能跑」的公开基线：clone / 自部署即用、质量稳定。
// owner 的持续 prompt 调优写进 git-ignored 的 `prompts.private.js`，由 `deploy.sh` 在部署时
// 临时替换上线、部署后还原——公开仓永远只留本 baseline。竞品拿到的永远是冻结的 baseline，
// 拿不到 owner 之后的迭代。详见 ../DERIVE.md「prompt 私有迭代」。
//
// ⚠️ 改本文件的「铁律」子串时，同步 core/telos_core/prompts.py：
//    workers/check-prompt-parity.mjs 校验两端在场，且若 prompts.private.js 存在则校验它也含全部铁律。
...
export const BLUEPRINT_USER = (goal) =>
  `目标：${goal}\n\n` +
...
export const MODULE_USER = (goal, modlist, goalName, mid, mtitle, msum, target) =>
...
export const CRITIQUE_USER = (goal, list) =>
...
// 概括标题（给旧项目补标题，轻量纯文本）。
export const TITLE_SYSTEM = "你把学习目标概括成一个简洁的【主题标题】，像课程名。只输出标题本身，不要引号、不要标点结尾、不要解释。";
...
export const LESSON_STEPS_JSON =
...
// ⚠️ 改这块要同步 core/telos_core/prompts.py 的 _STYLE_RULES（check-prompt-parity.mjs 会校验两端在场）。
...
export const LESSON_USER = (name, domain, prereqs, goal) => {
  const head = `知识点：${name}\n所属目标：${goal}\n学习类型(domain)：${domain}\n学习者已掌握的前置：${
...
export const PROBES_USER = (items, goal) =>
  `目标：${goal}\n知识点列表(id ｜ 名称 ｜ 类型)：\n${items}\n\n` +
