...
- **付费系统（Telos Pro）**：
  ...
  - **模板店（知识付费）**：`lib/telos/templates.ts`（3 个官方精修图谱：考研英语¥19.9 / Python 面试¥19.9 / 科二免费引流，每节点全配 drill/benchmark/阶段）+ `/store` 页（大纲预览/导入/购买/Pro 月领）。获取=免费 | 已购(app_metadata.telos_templates，webhook `tpl_*` SKU 发货·读改写合并) | Pro 每月领 1 个(本机记号 telos:tpl-claim，轻约束)。导入走 `telos:open-map` 一次性标记直达地图；受免费 3 项目上限约束。入口：引导页卡片 + /pro 权益。上新流程=往 TEMPLATES 数组加对象即可（雅思/CFA 待写）。
