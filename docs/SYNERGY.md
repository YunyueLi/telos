...
| **#1 FSRS → 面试复习** | ◑ 引擎已成、UI 待接 | **已交付且验证**：`greenroom/app/srs.js`（FSRS-4.5 逐行同构 Telos + cramClamp 倒计时夹紧 + 复习状态存取 + 稳定 ID，node/浏览器双用）；`tools/srs-parity.mjs` 对拍 Python fsrs.py **全绿**（3 序列 + interval + cramClamp）。已提交推送。**待接（下一步）**：`greenroom.html` 内联 srs.js（按 `<!--SRS-->` 约定 + `tools/embed-srs.py`）、加「复习」导航/视图/`renderReview()`、经历卡/逐字稿题新鲜度角标、i18n、CSS，并起控制台预览端到端验证。面试日取 `job.md` 时间线最近未来日期。 |
